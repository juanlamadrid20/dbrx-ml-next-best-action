...
class FeatureEngineer:
    ...
    def create_features(self) -> None:
        ...
        features_sdf = (
            ...
            .withColumn("avg_purchase_value",
                        F.when(F.col("num_purchases_last_month") > 0,
        ...
        features_sdf.write.mode("overwrite").format("delta").saveAsTable(self.config.feature_table_full)
    ...
    def _ensure_feature_constraints(self) -> None:
        ...
        if dups > 0 or nulls > 0:
            ...
            fixed.write.mode("overwrite").format("delta").saveAsTable(self.config.feature_table_full)

        # Set constraints
        try:
            self.spark.sql(f"ALTER TABLE {self.config.feature_table_full} ALTER COLUMN customer_id SET NOT NULL")
            constraint_name = f"{self.config.feature_table}_pk"
            self.spark.sql(f"ALTER TABLE {self.config.feature_table_full} ADD CONSTRAINT {constraint_name} PRIMARY KEY (customer_id)")
